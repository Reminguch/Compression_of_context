...
try:
    ...
    print(
        "FlexAttention not available. Please ensure you have PyTorch 2.5+ with FlexAttention support."
    )
    ...
    try:
        ...
        debug_print(
            f"  Current patcher: {getattr(st, 'CURRENT_PATCHER', 'Not available')}"
        )
...
class CompressedDecoderLayer(GradientCheckpointingLayer):
    ...
    def __init__(
        ...
        self.self_attn = CompressedAttention(
            layer_idx, T_w, r, config, M, enable_compression=enable_compression
        )
        ...
        self.attention_type = getattr(
            config,
            "layer_types",
            ["attention"] * getattr(config, "num_hidden_layers", 32),
        )[layer_idx]
    ...
    def forward(
        ...
        compressed_attention_mask: Optional[
            torch.Tensor
        ] = None,  # New parameter for compressed attention mask from previous layer
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        attn_output = self.self_attn(
            ...
            or attention_mask,  # Use compressed mask if available
        ...
        if self.mlp.gate_proj.weight.dtype != hidden_states.dtype:
            hidden_states = self.mlp(
                hidden_states.to(dtype=self.mlp.gate_proj.weight.dtype)
            )
            hidden_states = hidden_states.to(dtype=original_dtype)
        else:
...
class CompressedAttention(nn.Module):
    ...
    def compress_memory(
        self, x: torch.Tensor, T_w: int, r: float, M: Optional[int] = None
    ):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        # Compression of memory part # #  # noqa: E266
        if self.enable_compression:
            x_m, xm_cmp, x_w = self.compress_memory(
                hidden_states, self.T_w, self.r, self.M
            )
            ...
            if xm_cmp is None:
                ...
                dummy_cmp = self.compress(
                    hidden_states
                )  # Shape: (B, ⌊T/2⌋, C) or empty when T==1
                ...
                hidden_states = (
                    hidden_states + 1e-6 * dummy_scalar
                ...
                q_w_proj = self.q_proj(x_w).view(
                    B, self.config.num_attention_heads, T_w_actual, self.head_dim
                )
                q_w = self.q_norm(q_w_proj)

                km_cmp_proj = self.k_proj(xm_cmp).view(
                    B_m, self.config.num_key_value_heads, T_m, self.head_dim
                )
                km_cmp = self.k_norm(km_cmp_proj)

                FinalCompression = FinalCompressedTokens(
                    self.config, q_w, km_cmp, self.r, self.M
                )
        ...
        torch.arange(seq_len_compressed, device=hidden_states.device).unsqueeze(0)
        position_embeddings = create_position_embeddings_custom(
            seq_len_compressed, self.head_dim, hidden_states.device, working_dtype
        )
        ...
        if self.enable_compression:
            ...
            if attention_mask is None:
                ...
                attention_mask = attention_mask.unsqueeze(0).unsqueeze(
                    0
                )  # (1, 1, seq_len_current, seq_len_current)
            else:
        ...
        if self.use_flex_attention:
            ...
            block_mask = self.create_causal_block_mask(
                seq_len_current, hidden_states.device
            )
            if block_mask is not None:
                try:
                    ...
                    debug_print(
                        f"Block mask created with shape {getattr(block_mask, 'shape', 'unknown')} on {dev}"
                    )
        ...
        if attn_output is None:
            ...
            if self.num_key_value_groups > 1:
                ...
                key_states_std = (
                    key_states_std.unsqueeze(2)
                    .expand(bsz, kv_heads, self.num_key_value_groups, slen, hd)
                    .reshape(bsz, target_heads, slen, hd)
                )
                value_states_std = (
                    value_states_std.unsqueeze(2)
                    .expand(bsz, kv_heads, self.num_key_value_groups, slen, hd)
                    .reshape(bsz, target_heads, slen, hd)
                )
        ...
        if attn_output is None:
            raise RuntimeError(
                "Both FlexAttention and SDPA fallback failed; cannot continue training."
            )
        ...
        if self.enable_compression:
            if self.use_flex_attention:
                output_attention_mask = torch.tensor(
                    [seq_len_current], device=hidden_states.device, dtype=torch.long
                )
            else:
        ...
        )  # Return 5-tuple with compressed attention mask
    ...
    def __getattr__(self, name):
        ...
        if name in [
            ...
            raise AttributeError(
                f"'{self.__class__.__name__}' object has no attribute '{name}' (LoRA not supported)"
            )
...
class MLPCompression(nn.Module):
    def __init__(self, config: Qwen3Config):
        ...
        self.gate_proj = NoLoRALinear(
            2 * self.hidden_size, int(self.intermediate_size / 4), bias=False
        )
        self.up_proj = NoLoRALinear(
            2 * self.hidden_size, int(self.intermediate_size / 4), bias=False
        )
        self.down_proj = NoLoRALinear(
            int(self.intermediate_size / 4), self.hidden_size, bias=False
        )
    ...
    def forward(self, x):
        ...
        if T_even > 0:
            ...
            gate_pairs = (
                x_pairs
                if x_pairs.dtype == self.gate_proj.weight.dtype
                else x_pairs.to(self.gate_proj.weight.dtype)
            )
            up_pairs = (
                x_pairs
                if x_pairs.dtype == self.up_proj.weight.dtype
                else x_pairs.to(self.up_proj.weight.dtype)
            )
...
class FinalCompressedTokens(nn.Module):
    ...
    def importance_score(self):
        ...
        if self.num_key_value_groups > 1:
            ...
            km_cmp = km_cmp.unsqueeze(2).expand(
                batch_size, num_kv_heads, self.num_key_value_groups, seq_len, head_dim
            )
        ...
        cumulative_attention = attention_weights.sum(
            dim=2
        )  # Sum over query positions: (B, num_heads, T_m)

        importance_scores = cumulative_attention.mean(
            dim=1
        )  # Average over heads: (B, T_m)
...
def interleave_selected(
    x_m: torch.Tensor, xm_cmp: torch.Tensor, selected_indices: torch.Tensor
) -> torch.Tensor:
    ...
    if expand_mask.any():
        ...
        expand_indices = (
            torch.arange(num_selected, device=device).unsqueeze(0).expand(B, -1)
        )
